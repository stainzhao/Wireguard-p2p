...
agent = load_module("p2p_agent_v74", LINUX / "p2p_agent.py")
api = load_module("peers_api_v74", ROOT / "vps" / "peers_api.py")
daemon = load_module("portmap_daemon_v74", LINUX / "portmap_daemon.py")
candidates = load_module("candidates_v74", LINUX / "candidates.py")


class LowResourceDirectTests(unittest.TestCase):
    def setUp(self):
        agent.STATES = {}
    ...
    def test_release_and_low_resource_constants(self):
    ...
    def test_direct_health_requires_fresh_handshake_and_overlay_route(self):
        ...
        no_route = dict(local, allowed_ips=[])
        self.assertFalse(agent.direct_peer_healthy(no_route, "10.0.0.3"))
    ...
    def test_expired_control_session_preserves_healthy_direct(self):
        ...
        with mock.patch.object(
            agent, "local_wg_peers", return_value={key: local}
        ), mock.patch.object(agent, "wg_set") as wg_set, mock.patch.object(
            agent, "save_state"
        ):
    ...
    def test_explicit_disconnect_still_removes_direct(self):
        key = self._key()
    ...
    def test_vps_expiration_remove_is_marked_non_destructive(self):
        session = {
            "session_id": str(uuid.uuid4()),
            "key": self._key(),
            "ip": "10.0.0.3",
        ...
        with mock.patch.object(
            api, "signed_post", return_value={"ok": True}
        ) as signed:
